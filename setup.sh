...
BASE_DIR="$base_dir/themes/custom/$theme_name"
ORIGINAL_DIR=$(pwd)
...
if [ -f "$DEST_GRUMPHP_PATH" ]; then
  sed -i "s|bin: \"web/themes/custom/<theme_name>/node_modules/.bin/eslint\"|bin: \"web/themes/custom/$theme_name/node_modules/.bin/eslint\"|" "$DEST_GRUMPHP_PATH"
  sed -i "s|config: \".eslintrc.json\"|config: \"web/themes/custom/$theme_name/.eslintrc.json\"|" "$DEST_GRUMPHP_PATH"
  sed -i "s|bin: \"web/themes/custom/<theme_name>/node_modules/.bin/stylelint\"|bin: \"web/themes/custom/$theme_name/node_modules/.bin/stylelint\"|" "$DEST_GRUMPHP_PATH"
  sed -i "s|config: \".stylelintrc.json\"|config: \"web/themes/custom/$theme_name/.stylelintrc.json\"|" "$DEST_GRUMPHP_PATH"
